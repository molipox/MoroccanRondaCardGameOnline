...
const CardMaker = ({ref, CardNumber, CardType, zIndex, left, bottom, rotate, HandleClick, ChangeCards, cardsContainer, TablePosition, imageHeight, index,HandleDrag, handleRef,handleTableMiddle }) => {
  ...
  useGSAP(()=>{
        ...
        gsap.set(card.current, { display: "block",delay:index/5 }); 
        gsap.from(card.current, {duration:0.5,x: tableMiddleX,delay:index/5, y: tableMiddleY,ease:"power1.out",scale:0.8})
    ...
    useGSAP(() => {
      ...
      handleTableMiddle(tableMiddleX,tableMiddleY);
    },[]);
