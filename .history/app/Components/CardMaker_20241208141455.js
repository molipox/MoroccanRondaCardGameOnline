...
const CardMaker = ({CardNumber, CardType, zIndex, left, bottom, rotate, HandleClick, ChangeCards, cardsContainer, TablePosition, imageHeight, index,HandleDrag,}) => {
  ...
  useGSAP(()=>{
    ...
    useGSAP(() => {
      const ChangeMiddleY = (ChangeCardBottom - cardBottom)
      const ChangeMiddleX = (ChangeCardX - cardX) - (cardsContainerWidth * (left / 100));
      gsap.set(`.Card${CardNumber}${CardType}`, { display: "block",delay:index/5 }); 
      gsap.from(`.Card${CardNumber}${CardType}`, {duration:0.5,x: ChangeCardX,delay:index/5, y: ChangeMiddleY,ease:"power1.out",scale:0.8})
    },[]);
