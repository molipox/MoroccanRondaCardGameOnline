...
const Page = () => {
    ...
    setTimeout(() => {
        ...
        console.log(cardRef.current.length,playerhand.length);
            console.log(cardRef.current)
            cardRef.current.map((card,index) => {
                const classname = card.className.split(" ");
                console.log("tableMiddleX",tableMiddleX,"tableMiddleY",tableMiddleY)
                console.log(classname)
                gsap.set(`.${classname[5]}`, { display: "block",delay:index/5 }); 
                gsap.from(`.${classname[5]}`, {duration:0.5,x: tableMiddleX,delay:index/5, y: tableMiddleY,ease:"power1.out",scale:0.8})
            })
    }, 3000);
    let tableMiddleX;
    let tableMiddleY;
    function handleTableMiddle(tableMiddlex,tableMiddley){
        tableMiddleX = tableMiddlex;
        tableMiddleY = tableMiddley;
        console.log(tableMiddleX,tableMiddleY);
    } 
